    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            */
            ...
            marketingEngine.SendCustomerNotifications(customer);
        }
        public class Event
        {
        ...
        {
            private const int DAYS_BEFORE = 16;
            private const int MIN_DISTANCE = 400;
            ...
            public void SendCustomerNotifications(Customer customer)
            {
                ...
                DateTime dtBirthDay = new DateTime(dtNow.Year, customer.BirthDate.Month, customer.BirthDate.Day);

                if (dtBirthDay < dtNow.Date)
                {
                    dtBirthDay = dtBirthDay.AddYears(1);
                }

                List<Event> eventsBirthday = _events.Where(ev => dtBirthDay.Subtract(ev.Date).Days < DAYS_BEFORE && dtBirthDay.Subtract(dtNow).Days < DAYS_BEFORE && ev.Date <= dtBirthDay).ToList();

                List<Event> eventsCity = _events.Where(ev => ev.City == customer.City && ev.Date >= dtNow).ToList();


                List<string> closestCities = new List<string>();
                var dicCityDsitance = CalculateDistance(customer.City);

                if (dicCityDsitance.Any())
                {
                    closestCities = dicCityDsitance.Where(d => d.Value <= MIN_DISTANCE).Select(d => d.Key).ToList();
                }

                List<Event> eventsClosestCities = new List<Event>();

                if (closestCities.Any())
                {
                    eventsClosestCities = _events.Where(ev =>
                    {
                        return closestCities.Exists(ct => ct == ev.City) && ev.Date >= dtNow;
                    }).ToList();

                }

                if (eventsBirthday.Any())
                {
                    Console.WriteLine("Birthday Events");

                    foreach (var item in eventsBirthday)
                    {
                        Console.WriteLine($"{customer.Name} from {customer.City} event {item.Name} at {item.Date}");
                    }
                }

                if (eventsCity.Any())
                {
                    Console.WriteLine("Same city Events");

                    foreach (var item in eventsCity)
                    {
                        Console.WriteLine($"{customer.Name} from {customer.City} event {item.Name} at {item.Date}");
                    }
                }

                if (eventsClosestCities.Any())
                {
                    Console.WriteLine("Closest cities Events");

                    foreach (var item in eventsClosestCities)
                    {
                        Console.WriteLine($"{customer.Name} from {customer.City} event {item.Name} at {item.Date}");
                    }
                }
            ...
            private Dictionary<string, int> CalculateDistance(string customerCity)
            {
                var customerCityInfo = Cities.Where(c => c.Key == customerCity).FirstOrDefault();
                Dictionary<string, int> dicCityDistance = new Dictionary<string, int>();

                foreach (var item in Cities)
                {
                    var distance = Math.Abs(customerCityInfo.Value.X - item.Value.X) + Math.Abs(customerCityInfo.Value.Y - item.Value.Y);
                    dicCityDistance.Add(item.Key, distance);
                }

                return dicCityDistance;
            }
        }
    }
}